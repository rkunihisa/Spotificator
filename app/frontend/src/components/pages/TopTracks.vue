...
<template>
  <div class="min-h-screen bg-[#121212] py-10 px-2 sm:px-8 md:px-16 lg:px-32 font-sans">
    <div class="max-w-7xl mx-auto">
      ...
      <ul v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mt-6">
        <Track v-for="track in topTracks.items" :key="track.id" :track="track" />
      </ul>
    </div>
  </div>
</template>
